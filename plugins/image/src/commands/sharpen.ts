  ...
  imageCmd
    ...
    .action(async (input: string, options: SharpenOptions) => {
      ...
      try {
        ...
        const outputPath = options.output || path.join(process.cwd(), `${inputPath.name}-sharpened${inputPath.ext}`);
        ...
        if (options.dryRun) {
          ...
          console.log(chalk.green('✓ Would sharpen image:'));
        ...
        const pipeline = createSharpInstance(input).sharpen({
        ...
        spinner.succeed(chalk.green('✓ Image sharpened successfully!'));
        console.log(chalk.dim(`  Input: ${input}`));
        console.log(chalk.dim(`  Output: ${outputPath}`));
        ...
        spinner.fail(chalk.red('Failed to sharpen image'));
        if (options.verbose) {
