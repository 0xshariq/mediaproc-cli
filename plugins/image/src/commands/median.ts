  ...
  imageCmd
    ...
    .action(async (input: string, options: MedianOptions) => {
      ...
      try {
        ...
        const outputPath = options.output || path.join(process.cwd(), `${inputPath.name}-median${inputPath.ext}`);
        ...
        if (options.dryRun) {
          ...
          console.log(chalk.green('✓ Would apply median filter:'));
          console.log(chalk.dim(`  From: ${input}`));
          console.log(chalk.dim(`  To: ${outputPath}`));
          console.log(chalk.dim(`  Size: ${options.size || 3}`));
          return;
        }

        const metadata = await createSharpInstance(input).metadata();
        const pipeline = createSharpInstance(input).median(options.size || 3);
        ...
        spinner.succeed(chalk.green('✓ Median filter applied successfully!'));
        console.log(chalk.dim(`  Input: ${input}`));
        console.log(chalk.dim(`  Output: ${outputPath}`));
